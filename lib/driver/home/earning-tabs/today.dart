...
import 'package:cabby/rider/widget/constant.dart' as constant;
...
class _TodayState extends State<Today> {
  ...
  void total() async {
    ...
    final url = globalUrl + 'ride-total/' + id! + "/driverId/today";
    print(url);
...
class Summary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    String? cnvTime(time) {
      if (rideModel!.dropoffTime != null) {
        var dateTime = DateTime.parse(time);
        var h = DateFormat.H().format(dateTime);
      ...
      children: [
        Padding(
          ...
          child: SizedBox(
            child: Row(
              children: [
                Text('${cnvTime(rideModel!.dropoffTime)}'),
                const SizedBox(
                  width: 18,
                ),
                Expanded(
                    child: RichText(
                  text: TextSpan(
                      text: '${rideModel!.riderName}',
                      style: Config.style(FontWeight.bold, sColor!, 14),
                      children: <TextSpan>[
                        TextSpan(
                          text: '\n${rideModel!.paymentMethod}',
