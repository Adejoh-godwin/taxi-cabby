...
class BodyState extends State<Body> {
                  ...
                  children: [
                    ...
                    InkWell(
                      ...
                      child: CircleAvatar(
                        ...
                        backgroundImage:
                            _image == null ? imgUrl : FileImage(_image),
                        child: (_image == null)
                    ...
                    const SizedBox(height: 10.0),
                    buildTextFormField('Phone Number', 'number',
                        TextInputType.number, widget.userModel!.phone),
                    const SizedBox(height: 10.0),
                    Container(
                    ...
                    ElevatedButton(
                        onPressed: () {
                          if (imgUrl == 'null') {
                            displayToastMessage(
                                "Image is still uploading", context);
                            return;
                          }
                          setState(() {
                            loading = true;
  ...
  Future register() async {
    ...
    try {
      ...
      displayToastMessage(response['respnse'].toString(), context);
    } catch (err) {
      if (!mounted) {
        return;
      }
  ...
  saveImg(image) async {
    ...
    var userImageUrl =
        await storageReference.ref(imageFileName).getDownloadURL();
    ...
    // displayToastMessage('saved successfully', context);
  }
}
